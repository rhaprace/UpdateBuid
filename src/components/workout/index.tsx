...
const predefinedExercises: Exercise[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Lift by straightening your legs and hips.",
      "Lower the bar back down in control.",
    ],
  },
  {
    id: "5",
...
const Workout = () => {
  ...
  const recommendExercises = (user: any) => {
    ...
    if (goal === "Weight Loss") {
      filteredExercises = predefinedExercises.filter(
        (ex) => ex.level === "Beginner" || ex.bodyPart !== "Arms"
      );
    } else if (goal === "Gain Weight") {
      filteredExercises = predefinedExercises.filter(
        (ex) => ex.level === "Pro" || ex.bodyPart !== "Core"
      );
    }
    setExercises(filteredExercises);
  };
  ...
  return (
    <motion.div
      className="flex flex-col items-center min-h-screen p-5 h-auto bg-gradient-to-br from-[#000059] to-[#D9D9D9] text-white"
        ...
        {exercises.length > 0 ? (
          exercises.map((exercise) => (
            ...
            >
              ...
              <p
                className={`px-4 py-1 mt-2 rounded-full text-white ${
                  exercise.level === "Pro" ? "bg-red-500" : "bg-green-500"
                }`}
              >
                {exercise.level}
              </p>
